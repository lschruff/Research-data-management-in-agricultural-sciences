# Research Data Management in Agricultural Sciences - Advanced module: Data documentation – Trainingsmaterial

## Content

As part of the series ‘Tailored Research Data Management Training for Agricultural Scientists’ in cooperation between FAIRagro, the Consortium for the Agrosystems Research Community of the National Research Data Infrastructure (NFDI) and the Research Data Service of the University of Kassel, an in-depth workshop on Data documentation was held at 10th of December 2024, which specifically addressed the special features for agricultural scientists. This publication is intended to enable the subsequent use of the teaching materials and includes this README file, which also functions as a navigation aid for the individual files. The Teachingscript (in German only), helps with the conception, division of labour and timing of the introductory lecture. This and the presentation slides are available in various formats for easy reuse.

- **Target group of the workshop:** Researchers, focus on young scientists
- **Competence level:** Basic level, Intermediate level
- **Domain:** Agricultural sciences
- **Duration:** 2 h
- **Learning resource type:** Presentation slides, teaching script
- **Formats:** pdf, pptx, odp, xlsx, ods

## Navigation

- 1. [README](README_DataDocumentation_agri.pdf)
- 2. the teachingscript:
  - a. [Teachingscript_DataDocumentation_agri.pdf](teaching_script/Teachingscript_DataDocumentation_agri.pdf)
  - b. [Teachingscript_DataDocumentation_agri.ods](teaching_script/Teachingscript_DataDocumentation_agri.ods)
  - c. [Teachingscript_DataDocumentation_agri.xlsx](teaching_script/Teachingscript_DataDocumentation_agri.xlsx)
- 3. the presentationslides:
  - a. [Slides_DataDocumentation_agri.pdf](presentation_slides/Slides_DataDocumentation_agri.pdf)
  - b. [Slides_DataDocumentation_agri.odp](presentation_slides/Slides_DataDocumentation_agri.odp)
  - c. [Slides_DataDocumentation_agri.pptx](presentation_slides/Slides_DataDocumentation_agri.pptx)

## License

Research Data Management in Agricultural Sciences - Advanced module: Data documentation – Trainingsmaterial © 2025 by Boße, S.; Jordan, S.; Schneider, G.; Vedder, L. is licensed under CC BY 4.0

## Citation

Boße, S.; Jordan, S.; Schneider, G.; Vedder, L. Research Data Management in Agricultural Sciences - Advanced module: Data documentation – Trainingsmaterial, 2025, DOI [10.4126/FRL01-006428850](https://doi.org/10.4126/FRL01-006428850)

## Funding

This work was created as part of the NFDI consortium FAIRagro (www.fairagro.net). We gratefully acknowledge the financial support of the German Research Foundation (DFG) – project number 501899475.

## Related Resources

This advanced module was held following an introductory lecture and another advanced module on Data Documentation. 

### Introductory Lecture

Boße, S.; Jordan, S.; Rey Mazón, E.; Schröder, K. Introduction to Research Data Management in Agricultural Sciences – Trainingsmaterial, 2025, DOI [10.4126/FRL01-006428881](https://doi.org/10.4126/FRL01-006428881)

### Workshop on Data Publication

Boße, S.; Jordan, S.; Rey Mazón, E.; Sahwan, W. Research Data Management in Agricultural Sciences - Advanced module: Data publication – Trainingsmaterial, 2025, DOI [10.4126/FRL01-006426818](https://doi.org/10.4126/FRL01-006426818)